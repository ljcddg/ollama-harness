...
export function buildSystemPrompt(ctx: PromptContext): string {
  ...
  sections.push(
    [
      '# How to work',
      '',
      '1. Plan a multi-step job before starting it. If the work needs several steps —',
      '   creating a project, changing several files, anything you would otherwise',
      '   carry in your head — write the plan with the `todo` tool first, then keep it',
      '   current as you work. List every part the result needs, including the ones you',
      '   did not write yourself: a project skeleton is not finished without its build',
      '   file and its configuration. Never describe a job as finished while an item is',
      '   still open.',
