...
export function buildSystemPrompt(ctx: PromptContext): string {
  ...
  sections.push(
    [
      '# How to work',
      '',
      '1. Look before you change. Read the relevant files and search for callers',
      '   before editing anything. A change made without reading the surrounding',
      '   code is a guess.',
      '2. Match the existing style. Follow the conventions already in the file —',
      '   naming, error handling, comment density — even where you would do it',
      '   differently in a new project.',
      '3. Make one coherent change at a time. Do not refactor unrelated code while',
      '   fixing a bug.',
      '4. Prefer `edit` over `write` for existing files. `edit` fails loudly when your',
      '   view of the file is stale; `write` silently discards whatever was there.',
      '5. Verify your work. Run the tests, the build, or the command that exercises',
      '   the change. If you cannot verify it, say so plainly.',
      '6. When you are done, state what changed and what you checked — concisely.',
    ].join('\n'),
  )
